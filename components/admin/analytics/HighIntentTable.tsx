      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <div className="flex items-center gap-2">
            <span className="rounded-lg bg-indigo-500/16 p-2 text-indigo-300"><UsersRound className="h-4 w-4" /></span>
        ...
        <table className="min-w-full text-left text-sm text-white/85">
          ...
          <tbody>
            {sortedRows.length === 0 ? (
              ...
              sortedRows.map((row) => (
                <tr key={`${row.audienceKey}-${row.lastSeenAt}`} className="border-t border-white/5 odd:bg-white/[0.02] hover:bg-white/[0.06]">
                  <td className="px-3 py-2.5 font-mono" title={row.audienceKey}>{shorten(row.audienceKey)}</td>
                  <td className="px-3 py-2.5 font-semibold text-spotify-400">{toTierLabel(row.audienceTier)}</td>
                  ...
                  <td className="px-3 py-2.5 font-semibold text-spotify-300">{row.qualified.toLocaleString()}</td>
